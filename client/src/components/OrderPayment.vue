<template>
  <v-container>
    <h1>결제정보</h1>
    
    <v-btn v-if="final !== 0"  class="kakaopay" :class="{ active: selectedPaymentMethod === 'kakaopay' }" @click="selectPaymentMethod('kakaopay')" :style="{ backgroundColor: selectedPaymentMethod === 'kakaopay' ? 'yellow' : 'gray' }">
      <img class="css-1wbfblw e1ewacwr0" src="data:image/svg+xml;base64," alt="카카오페이">
    </v-btn>
    <v-btn v-if="final !== 0" :class="{ active: selectedPaymentMethod === 'toss' }" @click="selectPaymentMethod('toss')" :style="{ backgroundColor: selectedPaymentMethod === 'toss' ? 'lightblue' : 'gray' }">
       <img class="" scr="../server/toss.svg" alt="toss">
    </v-btn>
    <v-btn v-if="final !== 0" :class="{ active: selectedPaymentMethod === 'kg' }" @click="selectPaymentMethod('kg')" :style="{ backgroundColor: selectedPaymentMethod === 'kg' ? 'pink' : 'gray' }">신용카드</v-btn>
    <v-btn v-if="selectedPaymentMethod !== ''" @click="Order" color="primary">주문하기</v-btn>
    <v-btn v-else color="primary" disabled>주문하기</v-btn>
    <!-- <v-dialog v-model="dialog" persistent width="auto">
      <template >
        <v-btn color="primary" :="openPaymentWindow"> 주문하기 </v-btn>
      </template>
      <v-card>
        <v-card-title class="text-h5">
          주문화면
        </v-card-title>
        <v-card-text>주문하시겠습니까?.</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green-darken-1" variant="text" @click="sucerssForm">
            주문하기
          </v-btn>
          <v-btn color="green-darken-1" variant="text" @click="dialog = false">
            취소하기
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog> -->
    <p v-if="paymentUrl">결제창 URL: {{ paymentUrl }}</p>
    <p v-if="errorMessage">에러 메시지: {{ errorMessage }}</p>
  </v-container>
</template>

<script>
export default {
  name: 'OrderPayment',
  data() {
    return {
      selectedPaymentMethod: false,
      paymentUrl: '',
      errorMessage: '',
      dialog: false,
    };
  },
  props: {
        cartList: {
            type: Array,
            required: true,
        },
        final : 0,
        Order : {
            type: Function
        }
  },
  created(){
  },
  methods: {
    selectPaymentMethod(paymentMethod) {
      this.$emit('selectedPayMethod', paymentMethod);
    }
  },
};
</script>

<style>
.kakaopay {
    -webkit-box-align: center;
    align-items: center;
    -webkit-box-pack: center;
    justify-content: center;
    position: relative;
    width: 100%;
    height: 48px;
    border: 1px solid rgb(246, 229, 0);
    font-size: 14px;
    color: rgb(255, 255, 255);
    background: pink;
    font-weight: 500;
    border-radius: 3px;
}
</style>